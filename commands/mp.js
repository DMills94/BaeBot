...
\`https://osu.ppy.sh/community/matches/50005504 +w b:hr1 b:dt2 b:nm1 b:nm2 r:99 r:55 fp:1 grand finals xx\`
    ...
    __**Breakdown -- NB: TEAM 1 IS RED, TEAM 2 IS BLUE (As it appears on the MP / Lobby team order)**__
        
**URL** - MP url, derr
**+w** - include if the match has warmups!
**b:[text]** - include as many of these as there are bans. Order alternates, eg. the example has TEAM 1 banning \`HR1 and NM1\`
...
${deleteMessage ? `This message will delete itself in 1 minute to save space! Type \`${prefix}mp -save\` to stop this! (For pinning, etc)` : ''}
        ...
        m.delete()
            .catch(() => {
                m.author.send(`Psst, I can delete your \`${prefix}mp\` message but I need the __Manage Messages__ role in the channel! Please contact the server owner, or @Bae#3308 to give me that 😄`)
            })
    }
}
